...
def train_source(args):
    ...
    if args.net[0:3] == 'res':
        ...
        # 只微调最后一层（假设我们想微调最后一个block）
        for param in netF.layer4.parameters():
            param.requires_grad = True  # 解冻最后一个block的参数
    ...
    param_file = open("param.txt", "w")

    def print_param_info(name, param):
        l2_norm = torch.norm(param.data).item()
        param_file.write(f"Finetuning parameter: {name}, Size: {param.size()}, L2 Norm: {l2_norm}\n")

    # 仅为未冻结的参数设置优化器
    for k, v in netF.named_parameters():
        if v.requires_grad:
            param_group += [{'params': v, 'lr': learning_rate * 0.1}]
            print_param_info(k, v)
            # print(f"Finetuning parameter in netF: {k}, Size: {v.size()}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str, default='./object/office-home')
